...
async function bootstrap() {
    ...
    app.useGlobalPipes(
        new ValidationPipe({
            ...
            disableErrorMessages:
                configService.get('server.env') === 'production',
        }),
    );
    ...
    await app.listen(
        ...
        () => {
            ...
            loggerService.log(
                `Environment Name: ${configService.get('server.envName')}`,
            );
            loggerService.log(
                `     Environment: ${configService.get('server.env')}`,
            );
            loggerService.log(
                `  Listening port: ${configService.get('server.port')}`,
            );
            loggerService.log(
                `=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=`,
            );
